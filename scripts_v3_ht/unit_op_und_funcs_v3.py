...
class UnitOpUndFuncSampler():
    ...
    def get_all_targets(self, image_sampler:ImageSampler): #copy all the targets and label them
    ...
    def leastcolor_image(self, image_sampler:ImageSampler): # for each target in image, label pixels of least frequent color
        targets = image_sampler.get_all_targets()
        labeled_targets = []
        for target in targets:
    ...
    def mostcolor_image(self, image_sampler:ImageSampler): # for each target in image, label pixels of most frequent color
        targets = image_sampler.get_all_targets()
        labeled_targets = []
        for target in targets:
    ...
    def get_uppermost_image(self, image_sampler:ImageSampler): # find the target that sits highest in the image and label it
        targets = image_sampler.get_all_targets()
    ...
    def get_lowermost_image(self, image_sampler:ImageSampler): # find the target that sits lowest in the image and label it
        targets = image_sampler.get_all_targets()
    ...
    def get_leftmost_image(self, image_sampler:ImageSampler): # find the target that sits furthest left in the image and label it
        targets = image_sampler.get_all_targets()
    ...
    def get_rightmost_image(self, image_sampler:ImageSampler): # find the target that sits furthest right in the image and label it
        targets = image_sampler.get_all_targets()
    ...
    def get_func(self, assigned_op_name=None, assigned_op_arg=None):
        ...
        if op_arg == "none":
            if func == "check_symmetry":
                ...
                op_arg = random.choice(self.all_shape_types)
            elif func in ("leastcolor_target", "mostcolor_target"):
                op_arg = random.choice(self.all_colors + ["none"])
