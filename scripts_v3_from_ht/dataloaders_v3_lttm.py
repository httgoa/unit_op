...
class UnitOpDataset(Dataset):
    ...
    def _generate_target_pairs(self, func_type, op_name, op_arg, bg_color, target_cnt, bbox_min, bbox_max):
        ...
        unchanged_pairs = []
        loop_cnt =0
        # und functions that always produce a labeled (changed) result need no unchanged_pairs
        always_changed = func_type == "gen" or op_name in self.und_func_sampler.always_changed_funcs
        
        def eval_continue_condition():
            if always_changed:
                continue_condition = len(changed_pairs) < target_cnt
            else: # check_* style und functions can produce both changed and unchanged
        ...
        while eval_continue_condition():
            ...
            while not success:
                if loop_cnt > target_cnt*2:
                    ...
                    elif op_name == "check_symmetry" and op_arg!="none":
